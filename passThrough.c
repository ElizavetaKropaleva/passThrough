...
VOID 
DisconnectNotifyCallback (IN PVOID ConnectionCookie)
{
...
--*/
{
    ...
    DbgPrint("RtlInitUnicodeString. name = 0x%x\n", portName);
...
--*/
{
    ...
    {
        FltCloseCommunicationPort(gServerPort);
        DbgPrint("FltCloseCommunicationPort success");
    }
...
--*/
{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    wchar_t* x = L"kemtest";
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"kemtest");
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        //DbgPrint("FILE: ", Data->Iopb->TargetFileObject->FileName);
        if (RtlEqualUnicodeString(&required_extension, &NameInfo->Extension, FALSE))
        {
            //DbgPrint("TRUE");
            uint8_t* key = (uint8_t*) key_ch;
            uint8_t* iv = (uint8_t*) iv_ch;
            ...
            if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
                ...
                if (Data->Iopb->Parameters.Write.WriteBuffer != "") {
                    DbgPrint("ENCRYPT");
                    //uint8_t key_1[16] = { (uint8_t)0x2b, (uint8_t)0x7e, (uint8_t)0x15, (uint8_t)0x16, (uint8_t)0x28, (uint8_t)0xae, (uint8_t)0xd2, (uint8_t)0xa6, (uint8_t)0xab, (uint8_t)0xf7, (uint8_t)0x15, (uint8_t)0x88, (uint8_t)0x09, (uint8_t)0xcf, (uint8_t)0x4f, (uint8_t)0x3c };
                    //uint8_t iv_1[16] = { (uint8_t)0x00, (uint8_t)0x01, (uint8_t)0x02, (uint8_t)0x03, (uint8_t)0x04, (uint8_t)0x05, (uint8_t)0x06, (uint8_t)0x07, (uint8_t)0x08, (uint8_t)0x09, (uint8_t)0x0a, (uint8_t)0x0b, (uint8_t)0x0c, (uint8_t)0x0d, (uint8_t)0x0e, (uint8_t)0x0f };
                    
                    uint8_t hex_text[1024];
                    memset(hex_text, 0, 1024);
                    for (int i = 0; i < strlen(Data->Iopb->Parameters.Write.WriteBuffer); i++) {
                        hex_text[i] = (uint8_t)((char*)Data->Iopb->Parameters.Write.WriteBuffer)[i];
                    }

                    struct AES_ctx ctx;
                    AES_init_ctx_iv(&ctx, key, iv);

                    AES_CBC_encrypt_buffer(&ctx, hex_text, 256);

                    for (int i = 0; i < 1024; i++) {
                        ((char*)Data->Iopb->Parameters.Write.WriteBuffer)[i] = hex_text[i];
                    }

                    DbgPrint("ENCRYPT_TEXT: ");
                    DbgPrint(Data->Iopb->Parameters.Write.WriteBuffer);
                }
                ...
                if (Data->Iopb->Parameters.Read.ReadBuffer != "") {
                    ...
                    for (int i = 0; i < strlen(Data->Iopb->Parameters.Read.ReadBuffer); i++) {
                        hex_text[i] = (uint8_t)((char*)Data->Iopb->Parameters.Read.ReadBuffer)[i];
                    }

                    struct AES_ctx ctx;
                    AES_init_ctx_iv(&ctx, key, iv);

                    AES_CBC_decrypt_buffer(&ctx, hex_text, 1024);

                    for (int i = 0; i < strlen((char*)Data->Iopb->Parameters.Read.ReadBuffer); i++) {
                        ((char*)Data->Iopb->Parameters.Read.ReadBuffer)[i] = hex_text[i];
                    }

                    DbgPrint("DECRYPT_TEXT: ");
                    DbgPrint(Data->Iopb->Parameters.Read.ReadBuffer);
                }
            }
        }
